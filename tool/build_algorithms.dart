...
Future<void> main(List<String> args) async {
  ...
  if (repoArg == null) {
    stderr.writeln(
      ...
      '[--compile-worker] [--no-pub-get] [--force] [--native-snapshot]',
    );
    exitCode = 64;
    return;
  }
  ...
  final nativeSnapshot = args.contains('--native-snapshot');
  ...
  final generated = Directory('${root.path}/worker/generated');
  _replaceDirectory(stage, generated);

  if (nativeSnapshot) {
    _writeNativeSnapshot(
      root,
      generated,
      buildId,
      definitions,
...
String _compiledFingerprint(Directory root, Directory stage) {
  final files = <File>[
    ...
    File('${root.path}/lib/algorithm_worker_core.dart'),
...
List<File> _toolAndWorkerInputs(Directory root) => <File>[
  for (final path in [
    ...
    'tool/model.dart',
    'worker/main.dart',
    'lib/algorithm_worker_core.dart',
...
void _writeNativeSnapshot(
  ...
  final stage = Directory('${root.path}/lib/generated_next');
  if (stage.existsSync()) stage.deleteSync(recursive: true);
  stage.createSync(recursive: true);

  for (final file in generated.listSync().whereType<File>()) {
    if (!file.path.endsWith('.dart')) continue;
    file.copySync('${stage.path}/${file.uri.pathSegments.last}');
  }
  ...
  final encoded = base64Encode(utf8.encode(jsonEncode(catalog)));
  File('${stage.path}/catalog.g.dart').writeAsStringSync(
    '// Generated by tool/build_algorithms.dart. Do not edit.\n'
    "const generatedCatalogBase64 = '$encoded';\n",
  );

  _replaceDirectory(stage, Directory('${root.path}/lib/generated'));
}
