...
import 'sandbox_worker_runtime.dart';

typedef AlgorithmFactory = SortingAlgorithm Function(String id);

/// Platform-neutral implementation of the worker message protocol.
///
/// The web frontend wraps this in a JavaScript Web Worker; native Flutter
/// frontends run the same protocol inside a disposable Dart isolate.
class AlgorithmWorkerCore {
  AlgorithmWorkerCore({
    required this.post,
    required this.createVisualAlgorithm,
    required this.createBenchmarkAlgorithm,
  });

  final void Function(Map<String, Object?> message) post;
  final AlgorithmFactory createVisualAlgorithm;
  final AlgorithmFactory createBenchmarkAlgorithm;
  ...
  Future<void> _visualStart(
    ...
    session = CheckpointSession(
      emitFrame: (frame) => post({...frame, 'requestId': requestId}),
    );
    ...
    try {
      ...
      session.finish();
      post({'type': 'complete', 'requestId': requestId});
    } finally {
      _visualSession = null;
    }
  }
  ...
  void _analyze(String requestId, Map<String, dynamic> message) {
    ...
    final benchmarkCases = (message['cases'] as List)
        .map(
          (e) => (e as List).cast<num>().map((x) => x.toInt()).toList(),
        )
        .toList();
